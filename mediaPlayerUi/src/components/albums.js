...
const Albums = ({data}) => {
    ...
    return(
        ...
        <View style={{width:'70%',height:'100%',paddingVertical:windowWidth*0.02}}>
            <Text style={{fontSize:windowWidth*0.04,color:'white',marginBottom:windowWidth*0.01}}>{truncateText(data.name,28)} </Text>
            <View style={{flexDirection:'row',alignItems:'center'}}>
            <AntDesign name="folder1" size={windowWidth*0.03} color="white" style={{marginRight:windowWidth*0.02}} />
            <Text style={{fontSize:windowWidth*0.03,color:'white'}}>{`${data.songsLength} Song`}</Text>
            </View>
        </View>
        <TouchableHighlight
           onPressIn={()=>{}}
