...
const Artistee = ({data}) => {
    ...
    return(
        ...
        <Ionicons name="person" size={windowWidth * 0.15} color="white" style={{marginRight:windowWidth*0.03}} />
        <View style={{width:'70%',height:'100%',paddingVertical:windowWidth*0.02}}>
            <Text style={{fontSize:windowWidth*0.04,color:'white',marginBottom:windowWidth*0.01}}>{truncateText(data.Aname,28)} </Text>
            <View style={{flexDirection:'row',alignItems:'center'}}>
            <AntDesign name="folder1" size={windowWidth*0.03} color="white" style={{marginRight:windowWidth*0.02}} />
            <Text style={{fontSize:windowWidth*0.03,color:'white'}}>{data.no}</Text>
            </View>
        </View>
        <TouchableHighlight
           onPressIn={()=>{}}
